...
public class ExternalGridHandler : MonoBehaviour
{
    ...
    public void QuickDrop()
    {
        ...
        if (backpackBounds.Contains(position))
            ...
            dropSelection = Selection.Container;
        else
        ...
        if (selected != null && selected.name.Equals("visualIconContainer"))
        {
            ...
            else if (dropSelection == Selection.Container)
                    dropItem = container.inventory.grid[(int)Math.Round((-containerHolder.worldBound.position.y + selected.worldBound.position.y) / slotDimensions.width), (int)Math.Round((-containerHolder.worldBound.position.x + selected.worldBound.position.x) / slotDimensions.width)];
        }
        else
        {
            Debug.Log("No pick");
        ...
        if (dropItem != null)
        {
            if (dropItem.obj != null)
            {
                ...
                if (dropSelection == Selection.Container) container.inventory.AddToGrid(dropItem.location.height, dropItem.location.width, dropItem.dimensions.width, dropItem.dimensions.height, null);
                ...
                if (dropSelection == Selection.Container) container.inventory.itemList.Remove(dropItem);
        ...
        if (stats.equipmentInventory.backpack != null) LoadBackpackInventoryItems();
        if (stats.equipmentInventory.rig != null) LoadRigInventoryItems();
        if (container != null) LoadContainerItems();
    }
